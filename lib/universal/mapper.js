 ...
 *
 * @api public
 * @param {identify} screen
 * @param {Object} settings
 * @return {Object}
 */

exports.identify = function(identify, settings) {
  var result = createCommonGAForm(identify, settings);

  // return undefined if there were no mapped traits present
  var send;
  each(function(val, key) {
    if (result[shorten(val)]) send = true;
  }, extend(settings.dimensions, settings.metrics));
  if (!send) return;
...
exports.group = function(group, settings) {
  var result = createCommonGAForm(group, settings);
...
function createCommonGAForm(facade, settings) {
  ...
  if (timestamp) form.qt = (new Date()).getTime() - timestamp;
...
/**
 * Format track into GA-compatible key-value pairs.
 *
 * @param {Track} track
 * @param {String} action
 * @return {Object}
...
function formatEnhancedEcommerceEvent(track, action) {
  return compactObject({
    pa: action,
    t: 'event',
    ea: track.event(),
