...
function createCommonGAForm(facade, settings) {
  ...
  if (campaign.term) form.cm = campaign.medium;
  if (campaign.content) form.cc = campaign.content;
  if (screen.name) form.cd = screen.name;

  // Account for Mobile Spec which sends campaign data
  // on Install Attributed events through the properties
  if (facade.proxy('event') == "Install Attributed"){
    var campaign = facade.proxy('properties.campaign');
    if (campaign.name) form.cn = campaign.name;
    if (campaign.source) form.cs = campaign.source;
    if (campaign.medium) form.cm = campaign.medium;
